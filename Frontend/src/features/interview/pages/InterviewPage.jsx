...
function InterviewPage() {
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      ...
      const newCrossCount=crossCount+1
      
      // // STEP: Speak feedback then move forward
      // const feedback =
      //   result.quality === "good"
      //     ? 'Good answer.'
      ...
      if (
        result.quality === "weak" &&
        crossCount < MAX_CROSS &&
        result.crossQuestion
      ) {
        ...
        speakAndThen(
           ...
           isMuted,
          // `${feedback} let me ask a follow-up. ${result.crossQuestion}`,
          // () => {
          //   setCurrentQ(result.crossQuestion);
          //   setTimeout(()=>startListening(), 1000)
          // },
        ...
        if (nextIndex < questions.length) {
          ...
          speakAndThen(
             ...
             isMuted,
            // `${feedback} Moving to the next question. ${questions[nextIndex]}`,
            // () => {
            //   setCurrentQ(questions[nextIndex]);
            //   setTimeout(()=> startListening(), 1000)
            // },
          ...
          speakAndThen(
             "That was your final question. Great job!",
             ()=>handleFinish([...conversation, entry], [...scores, result.score]),
             isMuted,
            // `${feedback} That was your final question. great job! Saving your results now.`,
            // () =>
            //   handleFinish([...conversation, entry], [...scores, result.score]),
            // isMuted,
          );
          // // STEP-8: All done — calculate final score and save
          ...
          <div className="relative flex flex-col items-center gap-6 text-center">
            ...
            <div>
              ...
              <p className="text-on-surface-variant text-sm">
                Your AI interview is about to begin. Make sure you're in a quiet
                place.
              </p>
            </div>
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-8 relative min-h-0">
        ...
        {!isListening && !isSpeaking && !isEvaluating && transcript && (
           <div
             ...
             className="mt-4 px-8 py-3 bg-gradient-to-r from-primary to-secondary text-on-primary-fixed font-bold rounded-full text-sm shadow-[0_0_20px_rgba(163,166,255,0.3)] hover:scale-[1.02] active:scale-95 transition-all flex items-center gap-2 flex-shrink-0 cursor-pointer"
           >
