...
function Navbar() {
  ...
  //todo STEP-1: Get real user from context — no extra API call needed
  const { user } = useUser()
...
<button
  ...
  className='hover:bg-white/5 rounded-full p-2 transition-all'
>
  <span className='material-symbols-outlined text-on-surface-variant'>settings</span>
</button>

        {/*//* STEP-2: Real user initial */}
        <div className='w-8 h-8 rounded-full bg-gradient-to-br from-primary to-secondary flex items-center justify-center border border-primary/20'>
          <span className='text-xs font-bold text-on-primary-fixed'>{initial}</span>
        </div>
      </div>
    </header>
  )
